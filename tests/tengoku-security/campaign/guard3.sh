...
cd "$WORK" && git fetch -q origin && git reset -q --hard origin/main && git checkout -q -B tooling/guard-probe3 origin/main
printf '\n# campaign: second workflow touch during a cache build (publish guard test)\n' >> .github/workflows/queue-gate.yml
git commit -q -s -am "campaign: touch a workflow while a cache build runs (3)" && git push -q -f origin tooling/guard-probe3
gh pr create -R "$REPO" --head tooling/guard-probe3 --title "campaign: workflow touch during a build, take 3 (expect pass)" --body "publish-guard test" >/dev/null 2>&1
n=$(gh pr list -R "$REPO" --head tooling/guard-probe3 --json number -q '.[0].number'); git checkout -q -f main
for i in $(seq 1 20); do st=$(gh pr checks "$n" -R "$REPO" --json name,state -q '.[] | select(.name=="pr-gate") | .state'); [ "$st" = SUCCESS ] && break; sleep 30; done
log "guard3: PR #$n gate $st; queueing the PR first, dispatching the build once its merge group is running"
...
log "guard3: PR #$n queue entry $q at $(date -u +%H:%M:%S); dispatching the build"
...
log "guard3: PR #$n $(gh pr view "$n" -R "$REPO" --json state -q .state) at $(date -u +%H:%M:%S); build run $run1 $(gh run view "$run1" -R "$REPO" --json status -q .status)"
...
log "guard3: run1 $run1 → $(gh run view "$run1" -R "$REPO" --json conclusion -q .conclusion) | $steps1"
...
if [ "$run2" != "$run1" ]; then
  ...
  log "guard3: run2 $run2 (relaunched) → $(gh run view "$run2" -R "$REPO" --json conclusion -q .conclusion) | $steps2"
else log "guard3: no relaunched run found"; fi
log "guard3: releases now: $(gh release list -R "$REPO" | head -3 | awk '{print $3}' | tr '\n' ' ')"
log "GUARD2_DONE"
