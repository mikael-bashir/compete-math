...
\title{\textbf{To what extent does scaffolding improve agentic theorem\\
proving capacity in large models}}
...
\begin{abstract}
\noindent
We report results from three benchmark runs of the Leak (insert citation to https://www.competemath.com/about/leak) automated
...
finding is negative for our central design hypothesis: on identical 60-minute budgets and the same model, a single
...
(Ultra~Fleeting, \textbf{28/98, 28.6\%}); the difference is significant under an
exact sign test ($p \approx 0.013$). Decomposition contributed only two problems
the flat agent could not solve. We analyse where each approach wins, quantify a
difficulty cliff in FATE-X's top quartile (2/25 for both arms), document the
operational incidents that shaped the design, and lay out the follow-up
experiments the results motivate or that could not be completed due to budget.
\end{abstract}
...
The Leak stack is an agentic theorem-proving system for Lean~4 + Mathlib
\cite{demoura2021lean4,mathlib2020}, built around remote verification services
...
by parallel sub-agents. This report evaluates that bet empirically.

Between 28~July and 14~August 2026 we ran seventeen benchmark passes on FATE-X
\cite{fatex2025,benchmarklist_fatex}, recording every attempt as a structured
research row. The campaign was designed to answer four questions:

\begin{enumerate}[itemsep=1pt]
  \item \textbf{Absolute capability.} What fraction of a graduate-level algebra
        benchmark can the stack close under fixed wall-clock budgets?
  \item \textbf{Decomposition vs.\ flat proving.} Does the architect
        (blueprint\,$\to$\,node-provers\,$\to$\,refinement) pipeline beat a
        single continuous agent given the same model, budget, and verifier?
  \item \textbf{Strategy family ranking.} Within the Stronghold family, do the
        newer arms (Finality, Force, Forte) improve on Surround?
  \item \textbf{What does feedback buy?} Via the control ladder
        (\S\ref{sec:controls}): how much of the stack's performance comes from
        compiler error feedback, search, and cross-attempt memory respectively?
\end{enumerate}

Questions 1--3 are answered with data below. Question 4 is partially answered:
Control~III's full pass is still in progress, and its design was revised
mid-campaign (\S\ref{sec:c3}).

\paragraph{Summary of findings.}
(i)~Union coverage across all runs is \textbf{40/100} FATE-X problems.7
(ii)~The flat Control~II agent beats the best decomposition arm by 10 problems
on identical budgets ($p\approx0.013$); decomposition's unique contribution is
2 problems (\S\ref{sec:headtohead}).
(iii)~Within Stronghold, no newer arm matched Surround's 3/4 on the opening
slice; Finality, Force and Forte went a combined 1/12 on small probes
(\S\ref{sec:stronghold}).
(iv)~Both full-pass arms hit the same difficulty cliff: 2/25 in the hardest
quartile, and the \emph{same} two problems (\run{fatex\_083},
\run{fatex\_084}) (\S\ref{sec:difficulty}).
(v)~Two data-integrity bugs were found and fixed during the campaign; one
destroyed and then recovered 19 scored items (\S\ref{sec:incidents}).
...
researched and documented pipeline by Goedel Architect, changing the driving backbone -
instead of a fast and small LLM, such as DeepSeek-V4-Flash used by Goedel Architect,
we drive with a large agent (Claude Sonnet 5 Agent); a blueprint stage compiles a
skeleton of \texttt{sorry}-holes, node provers fill holes as ReAct-style tool
agents \cite{yao2023react} against live compiler feedback, and a refinement
stage restructures the skeleton between iterations. The interactive substrate
is Pantograph \cite{aniva2024pantograph,pypantograph}, extended with a
snapshot-based ``ghost state'' model \cite{shen2026snapshot,leakii2026upgrade}
that lets many agents branch one resident Lean daemon without expensive re-elaboration - while 
this service was used for Control-II runs and other experimental runs not presented in this report, 
it wasn't used for the Ultra Fleeting benchmark, in line with the design of
the borrowed architecture. In the further works section, we discuss some interesting
experiments that could not be completed before the writing of this report.
...
FATE-X ships as 100 Lean files pinning Lean \texttt{v4.28.0} \cite{fatex2025}.
...
\item \textbf{Toolchain confound.} Ultra verified against Mathlib
      ...
      directly comparable with the FATE-X leaderboard
      \cite{benchmarklist_fatex}.
\item \textbf{Budget heterogeneity across the ledger.} Budgets rose during
      the campaign (20\,$\to$\,30\,$\to$\,60 minutes), so cross-run
      comparisons outside the two full passes (e.g.\ Control~I at 30\,min vs.\
      Control~II at 60\,min) confound arm with budget.
\item \textbf{Difficulty rank as proxy.} The band analysis
      (\S\ref{sec:difficulty}) trusts FATE-X's stated ordering; we have not
      independently validated it.
\item \textbf{Contamination.} FATE-X sources include textbooks and exam
      archives that plausibly appear in model training data
...
\item \textbf{Notional costs.} Claude-driver dollar figures are
      API-equivalent estimates, not spend; they are comparable to each other
      but not to real-money baselines.
\end{enumerate}
