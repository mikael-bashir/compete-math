...
\title{\textbf{FATE-X Capability Benchmarks for the Leak Prover Stack:\\
Findings from the July--August 2026 Campaign}}
\author{Mikael Bashir}
\date{Draft --- 14 August 2026}
...
\begin{abstract}
\noindent
We report results from seventeen benchmark runs of the Leak automated
theorem-proving stack on FATE-X \cite{fatex2025}, a 100-problem graduate-level
abstract- and commutative-algebra benchmark in Lean~4 \cite{demoura2021lean4}.
The campaign covers all three strategy families (River, Ultra, Stronghold), a
three-arm control ladder, and two complete passes over the benchmark, at a total
recorded cost of \$1{,}832. The headline finding is negative for our central
design hypothesis: on identical 60-minute budgets and the same model, a single
...
operational incidents that shaped the protocol, and lay out the follow-up
experiments the results motivate.
\end{abstract}
...
(i)~Union coverage across all runs is \textbf{40/100} FATE-X problems.
...
hierarchical searches \cite{lample2022hypertree,decomposition_openreview}. Our
architect pipeline is an agentic variant: a blueprint stage compiles a
...
that lets many agents branch one resident Lean daemon without re-elaboration.
