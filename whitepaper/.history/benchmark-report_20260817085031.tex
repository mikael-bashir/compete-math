...
\begin{description}[itemsep=2pt]
      ...
      It is important to make clear that all of our strategy designs had their internet search tool stripped (from Claude's), so that
  ...
  \item[Ultra (Claude CLI driver).] A Goedeal Architect inspired pipeline driven by a local
        Claude CLI \cite{anthropic2026sonnet5} instance; \run{ultra-fleeting} has no recollection
        of proved lemmas between two seperate proof attempts and was not given NL proofs to shape decomposition.
  \item[Control~II (compiler + search).] A single continuous - meaning even after completion, prompted again if theorem is still unproven, on the same conversation session - Claude Sonnet~5
        agent \cite{anthropic2026sonnet5} armed with only Leak~IV (whole-script
        verification) and Leak~I (Loogle/Moogle lemma search
        \cite{loogle,moogle}); exactly how those tools are used is left to
        Claude's built-in harness \cite{anthropicharness}. Benchmarked as
        \run{control-oneshot-2}.
  \item[Control~III (blind prover).] The same base agent given \emph{no} additional tools (it still has Claude Code's built in tools, except internet search):
  ...
  \item[Control~IV (tool-rich, harness-driven).] The same base agent armed with
