...
const CONFIG_STORAGE_KEY = 'lca.config';
...
function loadConfig(): LocalClaudeConfigInput {
  try {
    const raw = localStorage.getItem(CONFIG_STORAGE_KEY);
    return raw
      ? { ...DEFAULT_LOCAL_CLAUDE_CONFIG, ...JSON.parse(raw) }
      : { ...DEFAULT_LOCAL_CLAUDE_CONFIG };
  } catch {
    return { ...DEFAULT_LOCAL_CLAUDE_CONFIG };
  }
}

function saveConfig(config: LocalClaudeConfigInput) {
  try {
...
export function LocalClaudeAgentManagement({
  ...
  // Load connection + preferences from localStorage when the dialog opens.
  useEffect(() => {
    if (!open) return;
  ...
  // Hand Claude the Lean MCP tools and let it loop until it produces a
  // verified proof of the theorem. Reuses the operator's registered MCP servers.
            ...
            <div className="space-y-5 py-1">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">
                    Non-secret settings, stored in this browser and sent to the
                    bridge per run.
                  </p>
                </div>
