...
// ── Bridge connection (same localStorage contract the Local Claude Agent
...
interface RunOpts {
  ...
  // the outcome are persisted to the admin debug log. Omit to skip logging
  // entirely — it's purely additive and never required for a prove to work.
  source?: string;
...
// Fire-and-forget: persist a run to the admin debug log, when that endpoint
// exists. Never lets logging affect the prove either way.
